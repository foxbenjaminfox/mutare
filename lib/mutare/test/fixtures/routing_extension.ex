defmodule Mutare.Test.Fixtures.RoutingExtension do
  ...
  @doc "Passes `expr` through unchanged. Routed fully `:raw` — its argument is opaque."
  defmacro opaque(expr), do: expr

  @doc "Passes `expr` through, discarding `label`. Routed `[:expression, :raw]`."
  defmacro tagged(expr, _label), do: expr

  @impl Mutare.CallRouting
  def call_routes do
    [
      {__MODULE__, :opaque, 1, :raw},
      {__MODULE__, :tagged, 2, [:expression, :raw]}
    ]
  end
end
