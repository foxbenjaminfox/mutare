defmodule Mutare.Test.Fixtures.RoutingExtension do
  ...
  Registering routes for its macros is how a library ships the knowledge its DSL relies on (`Mutare.MacroRouting`). Testing that your mutator honours routing from an *independent* module therefore needs a routed macro that module owns — this fixture is that module, so a plugin suite doesn't have to author a no-op routing provider of its own. It defines two inert pass-through macros and registers their argument routing:
  ...
  This is an extension — no `name/0`, no mutations — so it goes under `:extensions` (or a declarative `:macro_routes` entry naming its macros, to test the config channel). It deliberately covers only the end-user treatments: the adapter-grade ones (`:interpolated`, `{:keyword, …}`, `:hosted`) assert facts about a real DSL's semantics that no generic fixture can stand in for — model those on the `Mutare.MacroRouting` and `Mutare.Mutator.MacroHost` contracts against the library you're describing.
  """
