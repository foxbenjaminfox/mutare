defmodule Mutare.CallRouting do
  ...
  **A call is ordinary in every respect its route does not address.** Ordinary is how a plain
  ...
  `:lazy_expression` changes two things, and nothing else: the argument is never evaluated
  ahead of the call, and a name it binds **fresh** (`lazy(x = f())`, with no `x` bound before
  the call) is the callee's to scope, so Mutare does not carry it past the call's selector —
  under `:expression` it would. A name already bound before the call is carried either way,
  whatever the argument rebinds it to. So it is safe to declare wherever you are unsure,
  provided the position does not bind fresh names read after the call; the price is a larger
  metamutant for long chains of such stages.
  ...
  **A custom pipe hidden by macro expansion.** If Mutare cannot see the import that replaces
  ...
  Identical declarations from multiple code providers coalesce. Conflicting code-provided routes raise `Mutare.CallRouting.ContractError` instead of depending on configuration order. A declarative `call_routes:` entry is an explicit final override for its key, restricted to the user-tier vocabulary above; a configured `:skip` that displaces an adapter's hosted route is honoured (the user turned the call off), not reported as the adapter's contract violation. A configured `:skip` withholds mutation, not what the declaration it displaced — or shadowed, when a more specific skip wins over an any-arity, module-wide or name-only declaration — said the arguments *mean*: `destructure/2`'s pattern still binds and `match?/2`'s still binds nothing for the analysis that decides what an enclosing mutant must preserve, whether the skipped call is read directly or inside another skipped call's arguments — a static declaration is read as declared, a classifier the skip withholds (or providers that disagreed) as unknown, which withholds the dependent mutants. A configured positional route replaces the meaning too. The `--skip-call Module.fun/arity` flag is the CLI spelling of a `:skip` entry.
