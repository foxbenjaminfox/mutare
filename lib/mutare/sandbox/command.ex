defmodule Mutare.Sandbox.Command do
  ...
  **exit-code contract** — reading a run's exit code and output into a mutant
  ...
  once one file:
    ...
    * `Mutare.Sandbox.Command.Exit` — the exit codes themselves and their by-code
      reading (`Exit.decode/1`), the leaf both sides of the contract read from.
  ...
  *detected*-mutant cases from the otherwise-`:harness_error` bucket:
  ...
  last case, its output):
    ...
    * `:sigkilled` — exit `Exit.sigkill/0`: the OS SIGKILLed the run. A harness
  ...
  boot-skip flags `#{Enum.join(@boot_skip_flags, " ")}`:
    ...
    * `--exit-status #{Exit.failure()}` makes a clean test failure (a kill)
