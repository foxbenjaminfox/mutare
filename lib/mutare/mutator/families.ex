defmodule Mutare.Mutator.Families do
  ...
  `use` options:
    ...
    * `:opt_in` (default `[]`) — the subset excluded from the default set; each must be
      in `:all`.
    * `:plugin` (default the using module's name) — the plugin name included in error messages,
      e.g. `"Mutare.Ecto"`.
  ...
  mutation, on both delivery paths, so producers stay pure and every delivery site applies
  the filter.
  """
