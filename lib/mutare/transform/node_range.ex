defmodule Mutare.Transform.NodeRange do
  ...
  **no change at all** (a survivor with an empty diff). `get/1` adds back one
  ...
  Only `Mutare.Report` reads the range, so the quirk is invisible at runtime:
  the metamutant is built from the AST, never the range. It corrupts only a
  surviving mutant's rendered location — the human diff, and the `endColumn` the
  machine reporters emit (`Mutare.Report.Json` / `Mutare.Report.Sarif`).
  See NOTES "Sourceror range".
  """
