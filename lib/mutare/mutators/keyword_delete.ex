defmodule Mutare.Mutators.KeywordDelete do
  ...
  Matching is restricted to arity two. The deprecated three-argument `delete` form has no three-argument `delete_first` counterpart and is not mutated. Piped, direct, aliased, and imported calls are supported.
  ...
  # {alias_path, function, arity} => complementary function. Keyed on arity so the
  # swap only fires at /2 (`delete_first` has no /3 twin); `rebuild` keeps the written
  # module (the swap stays within `Keyword`).
  @rules %{
