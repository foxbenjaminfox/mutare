defmodule Mutare.Mutators.Helpers do
  ...
  @doc """
  Resolve a call and look `{module, fun, arity}` up in an arity-keyed `rules` map. (A `|>`
  stage reaches a mutator as the direct call it is sugar for, so the arity is the argument
  count.)
  ...
  def lookup_resolved_arity(node, rules) do
    with {module, fun, args, rebuild} <- Calls.resolved_call(node),
         {:ok, rule} <- Map.fetch(rules, {module, fun, length(args)}) do
  ...
  def swap_bare_kernel(_node, _table), do: :skip

  @doc """
  Remove a **bare `Kernel`** call, gated on arity.
  ...
  def remove_bare_kernel(_node, _removable), do: :skip

  @doc """
  Remove a *transparent transform* call.
  ...
  `removed_call/1`. Returns `:skip` when the node isn't a resolved call, or its
  ...
  """
  @spec remove_call(Macro.t(), MapSet.t({Calls.module_key(), atom()})) :: [Macro.t()] | :skip
  def remove_call(node, removable) do
  ...
  @doc """
  The removal itself, decoupled from how the call was matched: the call's first argument
  (`Enum.sort(x)` → `x`; written as a pipe, `x |> Enum.sort()` → `x`). `:skip` for a
  zero-argument call, which has nothing to return.
  """
  @spec removed_call([Macro.t()]) :: [Macro.t()] | :skip
  def removed_call([]), do: :skip
  def removed_call([first | _]), do: [first]
