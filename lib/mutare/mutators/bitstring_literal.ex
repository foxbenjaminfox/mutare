defmodule Mutare.Mutators.BitstringLiteral do
  @moduledoc """
  Bitstring-literal mutation: collapse a non-empty `<<…>>` literal to the empty bitstring `<<>>`. Like the corresponding list, map, and tuple mutations, this tests whether the binary's contents affect the test results. A binary that is built but whose bytes no test pins down lets `<<>>` survive.
  ...
  Not mutated — constructs that merely share the `<<…>>` AST shape:
    ...
    * a sigil's content (`~r/…/`, `~D[…]`) — the sigil mutators handle the whole node;
    * an interpolated quoted atom's content (the `<<>>` inside `:"a\#{x}b"`'s `binary_to_atom` call) — atom content, handled as a whole by `AtomLiteral`; the analyzer never offers the wrapper here.
