defmodule Mutare.Mutators.AliasLiteral do
  ...
  An alias is mutated only where it is a runtime *value* — `apply(MyModule, :f, [])`, `x = MyModule`, `[A, B]`, `is_struct(x, MyModule)`, a behaviour/strategy module passed as an argument. A weak suite that never pins down *which* module is used there lets the sentinel survive; anywhere the module is actually invoked, the mutant (a nonexistent module) raises and is killed.
