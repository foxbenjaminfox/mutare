...
A test that selects a mutant does so on a **selector key private to its module's execution**
...
- **`Mutare.Transform`** — the heart. `source → {metamutant_source, [%Site{}], next_id}` via an
  ...
  - **Name resolution pre-passes** (`Resolve` is the driver; `Aliases`, `Imports`, `Uses`,
    ...
    literally `:ok`/`nil`, and not a behaviour callback — a callback's `:ok` may be a contract
    outcome its runtime inspects) so no family is offered them — NOTES "Unit-returning functions
    are not return-value positions".
  ...
  - **Emit** (`emit/2`, plus pure helpers `ClauseAST` / `GuardBuild` / `LiftedEmit` /
    ...
    **function lifting + a dispatcher** (guards, head patterns, clause structure — where a `case`
    is illegal). Placement is positional; mutators never choose. A lifted clause's guard-only
...
- **The count pass is the scan's only parse.** Every scan-time diagnostic — ineffective/unknown
  `# mutare:` directives and their misplacement hint, ineffective `:skip_lifting` /
  `call_routes:` / `argument_marks:` entries, degraded `use`s for `--check` — reads *facts* the
  count worker returned through `Transform.count_report/2` (the `Ignore.Directives` container,
...
- **Two "leave it alone" facilities, deliberately split.** *Routes* (`call_routes:`,
  ...
  *Marks* (`argument_marks:`, `argument_marks/1`) label a
  ...
  displaced or shadowed says the arguments *mean*: `Registry.meaning/4` answers with the route
  that would govern the call were no skip configured, `RouteStamp.declared_routing/4` is the one
  reading of it for a stamped skip and for a call preserved beneath one, and both binding
  readers read every call through `Resolve.effective_routing/2` — NOTES "A configured skip
  displaces a route, not what the arguments mean", "A skip's meaning is one registry lookup".
...
| Identify the calls a DSL region embeds (a nested query, a registered macro) — the region is the host's syntax, so the host asks | `Mutare.Analyze.resolve/2` with the callback context, then `Mutare.Calls`' readers; shallow — a nested routed call's own regions need asking again | `analyze_test.exs` (`ResolvingHost`) |
