defmodule Mutare.Test.ReturnMutator do
  ...
  `c:Mutare.Mutator.return_replacements/1` (not `mutate/1`, which is `:skip`), so the transform
...
defmodule Mutare.Test.ConditionMutator do
  ...
  `c:Mutare.Mutator.condition_replacements/1`, so the transform offers it at every
