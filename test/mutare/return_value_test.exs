defmodule Mutare.ReturnValueTest do
  ...
  describe "duplicate node-level and return replacements" do
    ...
    test "counting agrees with emission and poison skips do not revive a duplicate" do
      ...
      for skipped_id <- [40, 41] do
        ...
        %{sites: rebuilt, next_id: rebuilt_next_id} =
  ...
  describe "which positions are targeted" do
    test "only the tail of a multi-statement body, not intermediate statements" do
      ...
      %{sites: sites} = Mutare.Transform.transform_string_with_sites(source, mutators: @only)
  ...
  describe "control-flow branch tails (case / cond / if / unless descended in tail position)" do
    ...
    defp hoisted_returns(source) do
      %{metamutant: meta, sites: sites} =
        Mutare.Transform.transform_string_with_sites(source, mutators: @hoisting)
