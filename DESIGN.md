...
| **Pattern swap** | swap two variables inside a `def`/`defp` head container — `{x, y}`→`{y, x}`, `[a, b]`→`[b, a]`, map values (structural, lifted) |
| **Pattern wildcard** | a variable that repeats in a `def`/`defp` head → replace one occurrence with `_`, dropping the equality constraint — `f(x, x)`→`f(_, x)` (structural, lifted) |
